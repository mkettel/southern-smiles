...
export default function OfficeRewardPreview() {
  ...
  const roster = ["Doctor", "Office member A", "Office member B", "Office member C", "Office member D", "Irma", "Evelis"].map((full_name, i) => ({
    id: `00000000-0000-4000-8000-00000000000${i + 1}`, full_name,
  }));
  const players = roster.filter(p => p.full_name !== "Irma");
  const guestPlayers = roster.filter(p => p.full_name === "Irma");
  const game: Game = {
    id: "00000000-0000-4000-8000-000000000010",
    title: "150 implants", detail: "Reach 150 implants placed during 2026, together.",
    audience: "Office", kind: "Reach a goal", members: roster.map(p => p.id),
    ...
    officeReward: { poolCents: 150000, units: Object.fromEntries(roster.map((p,i) => [p.id, i === 0 ? 2 : i === 6 ? 0.5 : 1])) },
  };
  return <main className="mx-auto max-w-7xl p-4 sm:p-8"><GamesWorkspace preview initial={{
    ...
    players, version: 0, data: { ...emptyGameData, guestPlayers, games: [game] },
  }} /></main>;
}
