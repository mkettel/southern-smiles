...
export function StatCard({ data }: StatCardProps) {
  ...
  const condition =
    currentEntry?.final_condition ??
    currentEntry?.auto_condition ??
    null;

  const delta =
    currentValue !== null && previousEntry
      ? currentValue - previousEntry.value
      : null;

  const percentChange = currentEntry?.percent_change ?? null;
            ...
            <>
              <div className="flex items-baseline gap-2">
                <span className="text-2xl font-bold">
                  {formatStatValue(currentValue, stat.stat_type)}
                </span>
                {delta !== null && (
