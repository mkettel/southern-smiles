...
import { loanProgress, monthlyPaymentEquivalent, type FinancialLoan, type FinancialLoansData } from "@/lib/financial-loans";
        ...
        <div className="min-w-0 overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="border-b bg-muted/35 text-left text-xs text-muted-foreground">
              <tr><th className="px-4 py-3 font-medium">Loan</th><th className="px-3 py-3 font-medium">Payment</th><th className="px-3 py-3 text-right font-medium">Balance</th><th className="px-4 py-3 font-medium">Progress</th></tr>
            </thead>
            <tbody>
              {initialData.loans.map((loan) => {
                ...
                return (
                  <tr key={loan.id} onClick={() => setSelectedId(loan.id)} className={cn("cursor-pointer border-b transition-colors last:border-0 hover:bg-muted/30", selectedId === loan.id && "bg-sky-50/70")}>
                    ...
                    <td className="min-w-32 px-4 py-3">{progress === null ? <span className="text-xs text-muted-foreground">Original balance needed</span> : <><div className="flex justify-between text-xs"><span>{progress}% paid</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-muted"><div className="h-full bg-emerald-600" style={{ width: `${progress}%` }} /></div></>}</td>
                  </tr>
                );
              })}
            </tbody>
...
function LoanDetail({ loan, accounts }: { loan: FinancialLoan; accounts: FinancialLoansData["liabilityAccounts"] }) {
  ...
  return <div className="space-y-5">
    ...
    <div><p className="text-xs text-muted-foreground">Current balance</p><p className="mt-1 text-2xl font-semibold tabular-nums">{money(loan.currentBalanceCents)}</p><p className="mt-1 text-xs text-muted-foreground">As of {dateLabel(loan.balanceAsOfDate)}</p></div>
...
function LoanEditor({ loan, accounts }: { loan?: FinancialLoan; accounts: FinancialLoansData["liabilityAccounts"] }) {
  ...
  async function submit() {
    const result = await saveFinancialLoan({
      ...
      originalPrincipalCents: cents(form.originalPrincipal), currentBalanceCents: cents(form.currentBalance) ?? 0,
