...
export function GamesWorkspace({ initial }: { initial: Awaited<ReturnType<typeof loadSurvivalGame>> }) {
  ...
  async function saveGame(event: FormEvent<HTMLFormElement>) {
    ...
    const game = { ...draft, id: draft.id || crypto.randomUUID(), goal: Number(values.get("goal")), current: Number(values.get("current")), points: Number(values.get("points")), scores: draft.kind === "Compete" ? draft.scores.map((_, i) => Number(values.get(`score-${i}`))) : draft.scores };
