...
import { type Game, type Reward, type Request, type Command, eligible, isIndividualCompetition, playerScore, applyGameCommand } from "@/lib/survival-game";
...
export function GamesWorkspace({ initial, preview = false }: { initial: Awaited<ReturnType<typeof loadSurvivalGame>>; preview?: boolean }) {
  ...
  const [scorePerson, setScorePerson] = useState(person);
  const [historyGame, setHistoryGame] = useState<Game | null>(null);
  const [showArchived, setShowArchived] = useState(false);
  useEffect(() => {
    if (preview || busy || draft || rewardDraft || submission) return;
    let active = true;
    const refresh = async () => {
  ...
  }, [preview, busy, draft, rewardDraft, submission]);
  ...
  async function commit(command: Command) {
    ...
    try {
      const result = preview
        ? { success: true as const, data: applyGameCommand(data, command, initial.actor, initial.players), version: version + 1 }
        : await changeSurvivalGame(command, version);
  ...
  async function submitProgress(event: FormEvent<HTMLFormElement>) {
    ...
    const ok = await commit({ type: "progress", id: submission.id, person: scorePerson, before: playerScore(submission, scorePerson), value: Number(values.get("value")), note: String(values.get("note") ?? "") });
  ...
  async function saveGame(event: FormEvent<HTMLFormElement>) {
    ...
    if (!draft) return;
    const values = new FormData(event.currentTarget);
    // Keep number fields editable (including empty) until the validated form is submitted.
    const game = { ...draft, id: draft.id || crypto.randomUUID(), goal: isIndividualCompetition(draft) ? draft.goal : Number(values.get("goal")), current: isIndividualCompetition(draft) ? draft.current : Number(values.get("current")), points: Number(values.get("points")), scores: draft.kind === "Compete" && !isIndividualCompetition(draft) ? draft.scores.map((_, i) => Number(values.get(`score-${i}`))) : draft.scores };
    if (await commit({ type: "saveGame", game })) setDraft(null);
  }
  ...
  return <fieldset disabled={busy} className="mx-auto max-w-7xl space-y-6 pb-12">
    ...
    {admin ? <>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4"><nav className="flex max-w-full gap-1 overflow-x-auto">{["My Games", "Teams", "Office", "Rewards"].map(t => <Button key={t} variant={view === t ? "default" : "ghost"} onClick={() => setView(t)}>{t}</Button>)}</nav><Button variant="ghost" onClick={async () => { try { const fresh = preview ? { data, version } : await loadSurvivalGame(); setData(fresh.data); setVersion(fresh.version); } catch { toast.error("Could not refresh games"); } }}>Refresh</Button></div>
      {view !== "Rewards" ? <>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{visible.map(g => <article key={g.id} className="flex min-w-0 flex-col rounded-lg border bg-card p-5"><div className="flex items-center justify-between gap-3"><span className="flex items-center gap-2 text-xs font-medium text-muted-foreground">{g.audience === "Individual" ? <Target className="size-4 text-sky-500" /> : g.audience === "Office" ? <Building2 className="size-4 text-emerald-600" /> : <Users className="size-4 text-emerald-600" />}{g.audience}</span><span className="text-xs font-semibold text-amber-700">{g.points} pts / person</span></div><h3 className="mt-4 text-lg font-semibold">{g.title}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{g.detail}</p>
          ...
          <Button className="mt-4 w-full" variant="outline" disabled={(!g.members.includes(person) && !(isAdmin && isIndividualCompetition(g))) || (g.kind === "Compete" && !isIndividualCompetition(g)) || (g.verification === "Manager entry" && !isAdmin) || (g.verification === "Manager approval" && pending.some(r => r.gameId === g.id && r.person === person))} onClick={() => { setScorePerson(g.members.includes(person) ? person : g.members[0]); setSubmission(g); }}>{(g.kind === "Compete" && !isIndividualCompetition(g)) || (g.verification === "Manager entry" && !isAdmin) ? "Manager scorekeeping" : !g.members.includes(person) && !isAdmin ? "Not participating" : g.verification === "Manager approval" && pending.some(r => r.gameId === g.id && r.person === person) ? "Awaiting approval" : <><Plus />Update progress</>}</Button><Button variant="ghost" className="mt-1 w-full" onClick={() => setHistoryGame(g)}>View history</Button></div></article>)}</div>
    ...
    <Dialog open={!!draft} onOpenChange={open => { if (!open) setDraft(null); }}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>{draft?.id ? "Edit game" : "Create game"}</DialogTitle></DialogHeader>{draft && <form onSubmit={saveGame} className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2"><label className="text-sm">Participants<select className={inputClass} value={draft.audience} onChange={e => setDraft({ ...draft, audience: e.target.value as Game["audience"], members: e.target.value === "Office" || (e.target.value === "Individual" && draft.kind === "Compete") ? people : [] })}>{["Individual", "Team", "Office"].map(s => <option key={s}>{s}</option>)}</select></label><label className="text-sm">Way to win<select className={inputClass} value={draft.kind} onChange={e => setDraft({ ...draft, kind: e.target.value as Game["kind"], members: e.target.value !== "Compete" && draft.audience === "Individual" ? draft.members.slice(0,1) : draft.members })}>{["Reach a goal", "Stay within a limit", "Build consistency", "Compete"].map(s => <option key={s}>{s}</option>)}</select></label></div>
      {draft.audience !== "Office" && <fieldset className="space-y-3 border-y py-3"><legend className="text-xs">{isIndividualCompetition(draft) ? "Competitors" : "Participants"}</legend>{isIndividualCompetition(draft) && <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={people.every(p => draft.members.includes(p))} onChange={e => setDraft({ ...draft, members: e.target.checked ? [...people] : [] })} />Everyone</label>}<div className="flex flex-wrap gap-4">{people.map(p => <label key={p} className="flex items-center gap-2 text-sm"><input type={draft.audience === "Individual" && !isIndividualCompetition(draft) ? "radio" : "checkbox"} name="member" checked={draft.members.includes(p)} onChange={e => setDraft({ ...draft, members: draft.audience === "Individual" && !isIndividualCompetition(draft) ? [p] : e.target.checked ? [...draft.members,p] : draft.members.filter(m => m !== p) })} />{name(p)}</label>)}</div></fieldset>}
      <div className="grid gap-4 sm:grid-cols-2"><label className="text-sm">Measurement<input required className={inputClass} placeholder="implants, reviews, dollars..." value={draft.unit} onChange={e => setDraft({ ...draft, unit: e.target.value })} /></label>{!isIndividualCompetition(draft) && <><label className="text-sm">{draft.kind === "Stay within a limit" ? "Budget limit" : "Goal quantity"}<input required type="number" min="0.01" step="any" className={inputClass} name="goal" defaultValue={draft.goal} /></label><label className="text-sm">Current total<input required type="number" min="0" step="any" className={inputClass} name="current" defaultValue={draft.current} /></label></>}<label className="text-sm">Reward points per participant<input required type="number" min="0" className={inputClass} name="points" defaultValue={draft.points} /></label><label className="text-sm">Start date<input required type="date" className={inputClass} value={draft.start} onChange={e => setDraft({ ...draft, start: e.target.value })} /></label><label className="text-sm">End date<input type="date" min={draft.start} className={inputClass} value={draft.end} onChange={e => setDraft({ ...draft, end: e.target.value })} /></label></div>
      ...
      {draft.kind === "Compete" && !isIndividualCompetition(draft) && <fieldset className="space-y-3 border-y py-3"><legend className="text-sm">Scoreboard</legend>{draft.sides.map((s,i) => <div key={i} className="grid grid-cols-2 gap-3"><label className="text-xs">Team {i+1}<input required className={inputClass} value={s} onChange={e => setDraft({ ...draft, sides: draft.sides.map((v,j) => j === i ? e.target.value : v) })} /></label><label className="text-xs">Score {i+1}<input type="number" min="0" step="any" required className={inputClass} name={`score-${i}`} defaultValue={draft.scores[i]} /></label></div>)}</fieldset>}
    ...
    <Dialog open={!!submission} onOpenChange={open => { if (!open) setSubmission(null); }}><DialogContent><DialogHeader><DialogTitle>Update progress: {submission?.title}</DialogTitle></DialogHeader>{submission && <form className="space-y-4" onSubmit={submitProgress}>{isIndividualCompetition(submission) && isAdmin && <label className="block text-sm">Participant<select className={inputClass} value={scorePerson} onChange={e => setScorePerson(e.target.value)}>{submission.members.map(p => <option key={p} value={p}>{name(p)}</option>)}</select></label>}<p className="text-sm text-muted-foreground">Current total: {playerScore(submission, scorePerson)} {submission.unit}</p><label className="block text-sm">New total ({submission.unit})<input className={inputClass} name="value" type="number" min="0" step="any" required key={scorePerson} defaultValue={playerScore(submission, scorePerson)} /></label><label className="block text-sm">Note (optional)<textarea className={cn(inputClass,"h-24 py-2")} name="note" maxLength={500} /></label><Button type="submit"><Check />{submission.verification === "Manager approval" ? "Submit for approval" : "Save update"}</Button></form>}</DialogContent></Dialog>
    <Dialog open={!!historyGame} onOpenChange={open => { if (!open) setHistoryGame(null); }}><DialogContent className="max-h-[85vh] overflow-y-auto"><DialogHeader><DialogTitle>{historyGame?.title}: history</DialogTitle></DialogHeader>{(data.updates ?? []).filter(u => u.gameId === historyGame?.id).length === 0 ? <p className="text-sm text-muted-foreground">No progress updates recorded yet.</p> : (data.updates ?? []).filter(u => u.gameId === historyGame?.id).map((u,i) => <div key={i} className="border-b py-3"><p className="text-sm font-medium">{name(u.person)}: {u.before} to {u.after}</p><p className="mt-1 text-xs text-muted-foreground">{new Date(u.at).toLocaleString()}{u.recordedBy && u.recordedBy !== u.person ? ` · Updated by ${name(u.recordedBy)}` : ""}</p>{u.note && <p className="mt-2 text-sm">{u.note}</p>}</div>)}</DialogContent></Dialog>
    <Dialog open={!!rewardDraft} onOpenChange={open => { if (!open) setRewardDraft(null); }}><DialogContent><DialogHeader><DialogTitle>{rewardDraft?.id ? "Edit reward" : "Add reward"}</DialogTitle></DialogHeader>{rewardDraft && <form className="space-y-4" onSubmit={async e => { e.preventDefault(); const values = new FormData(e.currentTarget); if (await commit({ type: "saveReward", reward: { ...rewardDraft, id: rewardDraft.id || crypto.randomUUID(), points: Number(values.get("points")) } })) setRewardDraft(null); }}><label className="block text-sm">Reward name<input required className={inputClass} value={rewardDraft.title} onChange={e => setRewardDraft({ ...rewardDraft,title:e.target.value })} /></label><label className="block text-sm">Point price<input required type="number" min="1" className={inputClass} name="points" defaultValue={rewardDraft.points} /></label><Button type="submit">Save reward</Button></form>}</DialogContent></Dialog>
  </fieldset>;
}
