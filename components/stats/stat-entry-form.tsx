...
interface EntryState {
  ...
  /** When a percentage value is between 0 and 1, ask the user to clarify */
...
export function StatEntryForm({
      ...
      setEntries((prev) => {
        ...
        if (statItem?.stat.stat_type === "percentage") {
          ...
          if (normalized > 0 && normalized < 1) {
            const asWhole = Math.round(normalized * 100 * 100) / 100; // e.g. 0.8 → 80
            if (asWhole <= 100) {
              // Recalculate condition for the value as-typed
  ...
  /** Resolve an ambiguous percentage — user picks either the decimal or whole-number interpretation */
  ...
  async function handleSubmit() {
    // Check for unresolved percentage ambiguities
    const hasAmbiguity = stats.some(
      (s) => entries[s.stat.id]?.percentageAmbiguity
    );
    if (hasAmbiguity) {
      toast.error("Please confirm all percentage values before submitting");
      return;
    }
            ...
            <CardContent className="space-y-3">
              <p className="text-sm text-muted-foreground">
                ...
                {statItem.previousWeekStart && (() => {
                  const missing =
                    weeksBetween(statItem.previousWeekStart, weekStart) - 1;
                  return (
