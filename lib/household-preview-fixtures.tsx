// Deterministic sample data for the dev-only household preview routes.

import { shiftMonthKey, type HouseholdAccountRow, type HouseholdSnapshotRow, type HouseholdTransactionRow } from "@/lib/household-finance";

export const previewAccounts: HouseholdAccountRow[] = [
...
/** Builds ~12 months of transactions and 30 days of balance snapshots ending at `today` (YYYY-MM-DD). */
export function buildPreviewFixtures(today: string, monthsBack = 12) {
...
export function previewToday(): string {
  return new Intl.DateTimeFormat("en-CA", { timeZone: "America/Phoenix", year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date());
}

export function PreviewChrome({ title, children }: { title: string; children: React.ReactNode }) {
  return (
      ...
      <main className="min-w-0 flex-1 overflow-y-auto px-4 py-6 sm:px-6 lg:px-8">
        <div className="mx-auto w-full max-w-[1500px]">
          <header className="mb-8 border-b pb-5">
            <h1 className="px-1 text-2xl font-semibold">{title}</h1>
          </header>
          {children}
        </div>
      </main>
